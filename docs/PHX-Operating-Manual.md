...
## Update — August 27, 2026 (sixth drop: venues, voices, the December gift)

- **Venue pages** (`#venue/<id>`): auto-built from public listings — name, address, directions, upcoming calendar with flyers and pricing, **＋ Follow venue** (follows widened to type 'venue'). Followers get ONE summary notification per venue per ingest run when new events land ("3 new events at Valley Bar"). Every venue name links here: event pages, map popups. Clearly labeled "auto-built from public listings" with a claim path (hello@thephx.app) — the Bandsintown/Yelp unclaimed-page playbook: build the audience first, pitch the venue with their follower count.
- **On-sale/price-drop/cancel alerts** (ingest v9): the nightly sync diffs each Ticketmaster event against what we knew — price minimum dropped → 💸 alert to everyone who RSVP'd (going AND interested); newly canceled → ❌ alert; rescheduled/postponed → 📅 alert.
- **Scraper fixes** (v8–v10, Jaye's report): the page readers were stripping `<img>` tags and hrefs before the model ever saw them — flyers and direct links were *impossible*. Now images extract FIRST (they live inside the event's `<a>`, which was deleting them), lazy-load `data-src` + CSS backgrounds included, entities decoded, and the model must return per-event direct links (never a calendar link) + the event's own flyer. **Merge, never clobber**: existing rows (drafts or published) only gain missing fields; human edits are never overwritten. Backfill run: 23 flyers + direct links landed on published events. Rebel Lounge's calendar is fully JS-rendered — a page reader can't see it (their TM listings still flow in); needs an API/feed source if we want their self-listed shows.
- **Realtime bell**: notification inserts stream to the recipient instantly (heads-up banner fires the second Ash/DMs/likes land); 45s poll stays as safety net.
- **DM voice notes + photos**: 📎 photo (vision-safety-checked like feed media) and 🎙️ hold-to-record voice notes (60s cap, mic prompt on first hold). Private `dm-media` bucket with per-thread-folder storage RLS — participants only, served via signed URLs. dm_send v3 carries kind/media_path; previews say "📷 Photo"/"🎙️ Voice note".
- **Caption editor**: "Aa Text" in the media preview — up to 3 draggable text lines (shadow / dark pill / ember pill styles, tap Aa to cycle), typed in place, **baked into the exported pixels** for both photos and clips via the same canvas pipeline as filters/stamp.
- **602 Recap**: `my_602_recap(year)` RPC + full story page (minutes, streams, days active, top artists/tracks, the city's year) + share. Card sits on the Music view — **supers see it year-round (founder preview), everyone sees it in December**. The Wrapped moment is wired and waiting.
- **Basemap**: CARTO began watermarking "api required" — swapped both maps to Esri Dark Gray Canvas (keyless, dark, attribution included). No API key needed; Stadia/MapTiler with a free key is the upgrade path if we ever want prettier tiles.

---

## Update — August 27, 2026 (THE AUDIT DROP — every green-lit item)

**Scraper overhaul (ingest v11–v13, sharded)**: pagination-aware page readers (RAK 10→33 events — the site serves 10/page and we only ever read page 1), truncation-salvage JSON parsing (Rebel Lounge 0→21 — its 84-event SeeTickets embed overflowed the model's output), per-source found-counts in every run log, failed sources rotate, ICS feed source type (method='ics'), sharded crons (tm nightly+evening, pages ×3, ics) after the combined run hit the edge compute ceiling.

**Everything shipped from the deep audit**:
- *Auth*: OAuth invite-gate VERIFIED already enforced (access wall on every auth event); Turnstile fully wired, dormant until `platform_settings.turnstile_site_key` is set; 13+ DOB gate at signup (birthdate → profiles); email-change flow; artist genre/city in editor; boot veil already pulsed (audit corrected).
- *Feed*: CDN image transforms w/ kill-switch fallback, cursor pagination + earlier-comments + infinite clips, edit/delete own posts (+"edited" label), repost flatten, hashtag word-boundary, scroll preserve, image alt text, **photo carousels** (2–5, swipe + dots, per-photo vision check), link-unfurl deferred (noted).
- *Composer*: **scheduled posts** (schedule_post + 5-min cron publisher, 🕐 button, queue chip w/ cancel), IndexedDB media drafts (attachment survives app closes), **9:16 letterbox canvas** — landscape clips with captions bake to 9:16 with text on the bars, preview matches.
- *Clips*: double-tap like + 🔥 burst, speed chip (1/1.5/2/½×), creator analytics table (views/30s/completions per clip), follow-CTA end card, sound-filter scroll restore.
- *Music*: full listening history sheet, crossfade length chip (3/6/9/12s), Up-Next reorder, guest-openable playlists + venue pages (growth surfaces), city-now chip in the player, **LRC synced lyrics** (timestamped lines highlight + autoscroll).
- *DMs*: live inbox (realtime on threads), mic hidden where unsupported, photo downscale-before-send, **10-minute unsend** (+realtime delete repaint), passive frozen banner.
- *Profiles*: member photo+banner upload at last (moderated; update_my_profile + p_banner_url), identity-switcher discovery toast, **QR share cards**, verification request flow + Studio review queue.
- *Notifications*: **real per-type mutes** (filtered in my_notifications AND notify_push — caught + fixed a push-contract break mid-flight), burst grouping ("A, B + 2 more"), prefs sheet in Settings.
- *Studio*: **Platform pulse** (DAU/WAU/MAU, D7 retention, signup weeks, funnel, top posts), **feature-flag kill switches** (set_feature_flag + flag() client helper), venue impact 7/30/90 + CSV export + per-event 📊 drilldown, reports Pending/History tabs, **Ash approve dry-run** (row count in the confirm for UPDATE/DELETE).
- *Perf/sec*: CSP **enforced** (was report-only; widened for flyers/turnstile/youtube/insights), SW cache bump, feed videos preload=none, vision moderation on EVERY image upload path (flyers, avatars, banners, pages, hero videos), `moderation_fail_closed` kill-switch flag ready for open signup, edge rate limits (moderate-media 20/min, delete-account 3/hr, export 2/hr), focus-visible a11y + faint-contrast bump, client error beacon + Studio errors tile, **self-serve data export** (export-my-data fn + Settings row).
