...
- **2026-08-09 later:** one-code fix (artist codes backfilled with owner_user_id; My Pass is the only code surface — Earnings now points there), **Full-Artist deep analytics** (`artist_analytics` RPC, tier-gated by `tiers.capabilities->analytics`; Basic sees a teaser), **Studio analytics went interactive** (funnel steps open the actual people; top artists open a drill-down sheet), tier-capability wiring landed (Basic Artist 10-post/month DB trigger + friendly composer error; favorites caps were already live), **Ash** shipped (founder-only ops agent w/ approval queue), deep Studio Analytics page, watermarked clip export, flagship player, playlists/library, Settings, follow lists, Discover revamp. Full map: `PHX-Operating-Manual.md` (the everything-doc — keep it current; Ash reads it).
